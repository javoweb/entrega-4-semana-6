...
from apigateway.seedwork.dominio.repositorios import Mapeador
...
from apigateway.modulos.autenticacion.aplicacion.dto import UsuarioDTO

class MapeadorUsuarios(Mapeador):
    def obtener_tipo(self) -> type:
        return Usuario.__class__
    
    def entidad_a_dto(self, entidad: Usuario) -> UsuarioDTO:
        print("mapeador-infraestructura-entidad a dto")
        print(entidad.user)
        print(entidad.password)
        usuario_dto = UsuarioDTO(entidad.user,entidad.password)
        return usuario_dto

    def dto_a_entidad(self, dto: UsuarioDTO) -> Usuario:
        usuario = Usuario(dto.user, dto.password)        
        return usuario
