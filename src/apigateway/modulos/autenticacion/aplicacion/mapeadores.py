from apigateway.seedwork.aplicacion.dto import Mapeador as AppMap
from apigateway.seedwork.dominio.repositorios import Mapeador as RepMap
from apigateway.modulos.autenticacion.dominio.entidades import Reserva, Aeropuerto, Usuario
from apigateway.modulos.autenticacion.dominio.objetos_valor import Itinerario, Odo, Segmento, Leg
from .dto import ReservaDTO, ItinerarioDTO, OdoDTO, SegmentoDTO, LegDTO, UsuarioDTO
...
class MapeadorUsuarioDTOJson(AppMap):
    def externo_a_dto(self, externo: dict) -> UsuarioDTO:
        ...
        print(externo.get("username",None))
        usuario_dto: UsuarioDTO()
        usuario_dto= UsuarioDTO(externo.get("username",None),externo.get("password",None))
        return usuario_dto
...
class MapeadorUsuario(RepMap):
    def entidad_a_dto(self, entidad: Usuario) -> UsuarioDTO:
            usuario_ = entidad.user
            password_ = entidad.password
            _id = str(entidad.id)

            return UsuarioDTO(usuario_, password_)
